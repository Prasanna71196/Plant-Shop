...
if (filtered.length > 8) {
    renderProducts(filtered.slice(8, 12), 'second-section');
...
fetch('JSON/products8.json')
    ...
    .then(products => {
        ...
        renderProducts(products.slice(8, 12), 'second-section');
    })
    .catch(error => {
        console.error('Error loading products:', error);
    });

// Load header layout FIRST, then load cart popup
fetch("Components/header.html")
  .then(res => res.text())
  .then(data => {
    document.getElementById("header-placeholder").innerHTML = data;
    ...
    // Attach search functionality after header loads
    ...
    if (searchInput) {
      searchInput.addEventListener('input', function () {
        ...
        if (query) {
          ...
          renderProducts(allProducts.slice(8, 12), 'second-section');
          document.getElementById('second-section').style.display = 'grid';
